 ...
 */
$(function () {
    var div, jm, jmnodes, specific_options, mindmaps = $("div.diagram.mindmap");
    if (mindmaps.length) {
        loadResource( 'js/plugin/jsmind/jsmind.css', 'stylesheet', function() {});
        $.each($("div.diagram.mindmap"), function(index, diagram) {
		    ...
		    get_mindmap.onreadystatechange = function() {
			    if( get_mindmap.readyState === 4 ) {
				    if ( ( get_mindmap.status >= 200 && get_mindmap.status < 300 ) || get_mindmap.status === 0 ) {
                        ...
                        specific_options = JSON.parse(JSON.stringify(jsmind_options));
                        specific_options.container = diagram.id;
                        jm = jsMind.show(specific_options, jsmind_data);
                        div = $('div.jsmind-inner').last();
                        div.attr("id",diagram.id);
                        div.find('canvas').attr("id",diagram.id);
                        div.find('jmnodes').attr("id",diagram.id);
                        console.log(div.find('canvas'));
                        // NB: of course, jsmind_options is loaded in an apart js file
				    } else {
					    diagram.text('ERROR: The attempt to fetch "' + fn + '" failed with HTTP status ' + get_mindmap.status + '.' +
